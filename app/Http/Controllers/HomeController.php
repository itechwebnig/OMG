...
//my work
                ...
                foreach ($homeSub[$cat->id] as $sub) {
                 $homeproduct[$sub->id]=Items::orderBy('items.id','desc')->where('items.status',1)->where('fk_sub_category_id',$sub->id)->simplePaginate(8);
                    foreach ($homeproduct[$sub->id] as $key =>  $values) {
                        ...
                        $homeproduct[$sub->id][$key]['photo']=(isset($photos->photo))?$photos->photo:'';
                        $homeproduct[$sub->id][$key]['small_photo']=(isset($photos->small_photo))?$photos->small_photo:'';
                    }
                }
        ...
        return view('frontend.index', compact('allData','homeCategory','homeItems','photos','homeSub','totalItemInCat','all_category','all_sub_category','homecategory','homeproduct','special_product','products_photo','allblog','route','recentProduct','recent_photo','advertisement'));
    }
    ...
    /*public function itemPhoto(){
        ...
        foreach ($photos as $key => $value) {
            ItemPhoto::where('id',$value->id)->update([
                'photo'=>"images/bakejunction/item".$value->photo,
                'small_photo'=>"images/bakejunction/item/small".$value->photo,
            ]);
        }
        return "Success Sir!";
    }*/
