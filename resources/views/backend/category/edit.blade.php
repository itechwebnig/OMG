 ...
 <a href="{{route('categories.index')}}" class="btn btn-default pull-right"> <i class="ion ion-navicon-round"></i> View All</a></h3>
    {!! Form::open(array('route' => ['categories.update', $data->id],'method'=>'PUT','class'=>'form-horizontal','files'=>true)) !!}
        ...
        @endif
             ...
             <div class="form-group  {{ $errors->has('display_in_mainmenu') ? 'has-error' : '' }}">
            {{Form::label('display_in_mainmenu', 'Show in Mainmenu', array('class' => 'col-md-3 control-label'))}}
        <div class="col-md-6">
            <div class="col-md-3"><label class="btn btn-success"><input type="radio" name="display_in_mainmenu" value="1" {{($data->home_status==1)? 'checked' : ''}}> Yes </label> </div>
            <div class="col-md-3"><label class="btn btn-danger"><input type="radio" name="display_in_mainmenu" value="0" {{($data->display_in_mainmenu==0)? 'checked' : ''}}> No </label> </div>
        </div>
        @if ($errors->has('display_in_mainmenu'))
            <span class="help-block">
                <strong>{{ $errors->first('home_status') }}</strong>
            </span>
        @endif
