          ...
          <div class="navbar-collapse collapse" id="mc-horizontal-menu-collapse">
            <div class="nav-outer">
              <ul class="nav navbar-nav">
                ...
                <a href="#" data-hover="dropdown" class="dropdown-toggle" data-toggle="dropdown"> ALL CATEGORY <i id="icon_chearven" class="fa fa-chevron-down" aria-hidden="true"></i></a>
                      ...
                      <div class="yamm-content">
                        <div class="row">
                           ...
                           <div class="col-xs-12 col-sm-12 col-md-2 col-menu">
                            <h2 class="title"><a href="{{URL::to("category/$cat_name->link")}}">{{$cat_name->category_name}}</a></h2>
                            <ul class="links">
                              <?php $subcats=DB::table('sub_category')->where('fk_category_id',$cat_name->id)->get(); ?>
                              @foreach($subcats as $subcat_name)
                              <li><a href="{{URL::to("category/$cat_name->link/$subcat_name->link")}}">{{$subcat_name->sub_category_name}}</a></li>
                               @endforeach
                            </ul>
                          </div>
                          @endforeach
            ...
            <?php $category_name=DB::table('category')->where('display_in_mainmenu',1)->get(); ?>
             @foreach($category_name as $cat_name)
            <li class="dropdown mega-menu"> 
                <a href="{{URL::to("category/$cat_name->link")}}" data-hover="dropdown" class="dropdown-toggle">{{$cat_name->category_name}} <i id="icon_chearven" class="fa fa-chevron-down" aria-hidden="true"></i></a>
                  <ul class="dropdown-menu container">
                    <li>
                      <div class="yamm-content">
                        <div class="row">
                           <!-- category item -->
                          <?php $subcats=DB::table('sub_category')->where('fk_category_id',$cat_name->id)->get(); ?>
                            @foreach($subcats as $sub_cat_name)
                           <div class="col-xs-12 col-sm-12 col-md-2 col-menu">
                            <h2 class="title"><a href="{{URL::to("category/$cat_name->link/$sub_cat_name->link")}}">{{$sub_cat_name->sub_category_name}}</a></h2>
                            <ul class="links">
                            <?php $subchild=DB::table('sub_sub_category')->where('fk_sub_category_id',$sub_cat_name->id)->get(); ?>
                             @foreach($subchild as $subchild_name)
                              <li><a href="{{URL::to("category/$cat_name->link/$sub_cat_name->link/$subchild_name->link")}}">{{$subchild_name->sub_name}}</a></li>
                             @endforeach
                            </ul>
                          </div>
                          @endforeach
